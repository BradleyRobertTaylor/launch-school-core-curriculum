const ItemCreator = (() => {
  ...
  return function(itemName, category, quantity) {
    if (isValidItemName(itemName) && isValidCategoryName(category)
...
const ItemManager = {
  ...
  update(sku, obj) {
    const item = this.items.filter(({skuCode}) => sku === skuCode)[0];   
  ...
  inStock() {
    const items = this.items.filter(({quantity}) => quantity > 0);
    return items;
  },

  itemsInCategory(searchCategory) {
    const items = this.items.filter(({category}) => category === searchCategory);
    return items;
  }
};
